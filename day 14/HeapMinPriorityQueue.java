...
implements MinPriorityQueue<E> {
	...
	public E extractMin() {
		if (heap.size() <= 0)
			...
			E minVal = heap.get(0); //min will be at node 0
			heap.set(0, heap.get(heap.size()-1));  // Move last to position 0
			heap.remove(heap.size()-1); //remove last item to maintain shape property
			minHeapify(heap, 0); //recursively swap to maintain order property
	...
	minHeapify(ArrayList<E> a, int i) {
		...
		if (smallest != i) {
			...
			minHeapify(a, smallest); //maintain heap starting from smallest index (with largest item)
		}
	} 

	// Swap two locations i and j in ArrayList a.
	private static <E> void swap(ArrayList<E> a, int i, int j) {
		E temp = a.get(i); //temporarily hold item at index i
		a.set(i, a.get(j)); //set item at index i to item at index j
		a.set(j, temp); //set item at index j to temp
	}
	...
	public static void main (String [] args)  {
		HeapMinPriorityQueue<Integer> pq = new HeapMinPriorityQueue<Integer>();
		pq.insert(15);
		pq.insert(2);
		pq.insert(7);
		pq.insert(12);
