...
public class BoyerMoore {
	...
	public static int findBoyerMoore(char[] text, char[] pattern) {
	  ...
	  while (i < n) { 
		  if (text[i] == pattern[k]) { // match! return i if complete match; otherwise, keep checking.
			  ...
			  System.out.println("mismatch " + text[i] + " and " + pattern[k] + " at index i=" + i + " and k=" + k);
			  int temp = i;
			  i += m - Math.min(k, 1 + last.get(text[i]));  //move in text
			  System.out.println("\ti was " + temp +", k=" + k + ", 1+last="+(1 + last.get(text[temp]))+", m="+m+" moving to i=" + i);
  ...
  public static void main(String[] args) {
	  char[] text = "abczefabcdef".toCharArray();
	  char[] pattern = "abcdef".toCharArray();
