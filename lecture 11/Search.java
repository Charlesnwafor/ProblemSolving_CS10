 ...
 * @author Tim Pierson, CS 10, Winter 2017 -- added minor comments
...
public class Search {
	...
	Map<String,Integer> totalCounts;					// word -> count, total over all files
		...
		while ((line = in.readLine()) != null) {
			...
			for (String word: words) {
				...
				if (wordCounts.containsKey(word)) {
					// Increment the count
					wordCounts.put(word, wordCounts.get(word)+1); //get returns null if key not found
				}
				else {
					// Add the new word
	...
	public Set<String> search(String[] terms) {
		...
		for (String term : terms) 
			good.retainAll(search(term)); //see Java docs, this is an intersection
